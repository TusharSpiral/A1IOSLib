    ...
    private func checkForceUpdateNeeded(config: VersionConfig) -> Bool {
        ...
        if config.minVersion.compare(appVersion, options: .numeric) == .orderedDescending {
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                Utility.showAlert(title:config.forceTitle, message:config.forceMessage, defaultTitle: Localization.updateNow, defaultHandler: handler)
            }
            return true
        } else {
            return false
    ...
    private func checkOptionalUpdateNeeded(config: VersionConfig) -> Bool {
        ...
        if config.stableVersion.compare(appVersion, options: .numeric) == .orderedDescending {
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                Utility.showAlert(title: config.optionalTitle, message: config.optionalMessage, defaultTitle: Localization.maybeLater, defaultHandler: handler, isCancel: true, cancelTitle: Localization.updateNow, cancelHandler: handler1)
            }
            return true
        } else {
            return false
